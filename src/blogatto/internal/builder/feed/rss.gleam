...
pub fn build(
  output_dir: String,
  config: List(rss_feed_config.RssFeedConfig(msg)),
...
fn build_feed(
  output_dir: String,
  config: rss_feed_config.RssFeedConfig(msg),
  metadata: List(feed_config.FeedMetadata(msg)),
) -> Result(Nil, error.BlogattoError) {
...
/// Convert a `RssFeedItem` to an `rss.RssItem` for XML generation.
fn feed_item_to_webls(item: rss_feed_config.RssFeedItem) -> rss.RssItem {
  rss.RssItem(
    title: item.title,
...
/// Convert a `RssFeedConfig` channel fields to an `rss.RssChannel` for XML generation.
...
fn weekday_to_webls(day: rss_feed_config.Weekday) -> rss.Weekday {
  case day {
    rss_feed_config.Monday -> rss.Monday
    rss_feed_config.Tuesday -> rss.Tuesday
    rss_feed_config.Wednesday -> rss.Wednesday
    rss_feed_config.Thursday -> rss.Thursday
    rss_feed_config.Friday -> rss.Friday
    rss_feed_config.Saturday -> rss.Saturday
    rss_feed_config.Sunday -> rss.Sunday
  }
}
